%% improved DPC for any dimention input, with halo removing
clear 
close all
tic
...
%dim = size(input,2);
filename='Trips_Sync.txt';%S1 Spiral R15 Pathbased Jain Flame D31 Compound Aggregation;
path=['./data/',filename];
[lons,lats,id_cl]=textread(path, '%f,%f,%d');
dataPts=[lons,lats];
% dataPts=dataPts(1:10:end,:);
% id_cl=id_cl(1:10:end,:);
...
R=min(board);
DC=integral(sorted_den,ND,0.02);
dc = DC*unit;
...
filter_in = den_rep;
sigma_vec = dc;  %dc*2 dc*2 dc dc*2 dc/2 dc/2 dc dc/2.0 dc/1.2 dc
sigma_dim = dim;
...
for ii=2:ND_rep
    ...
    for jj=1:ii-1
        tmp_dist = norm(loc_rep(ordrho(ii),:)-loc_rep(ordrho(jj),:),1);       
...
%% with halo
 ...
 if (NCLUST>1)
    for i=1:ND_rep-1
        for j=i+1:ND_rep
            pos_diff = abs(loc_rep(i,:) - loc_rep(j,:));
            tmp_dist = norm(pos_diff,1);
...
%% plot the ground truth
 ...
 for i = 1:size(id_cl,1)
     if(id_cl(i)<1)
         plot(dataPts(i,1),dataPts(i,2),tchar(ceil((max(id_cl)+1)/3)),'MarkerSize',4,'MarkerFaceColor',cmap(mod(max(id_cl)+1,3)+1,:),'MarkerEdgeColor',cmap(mod(max(id_cl)+1,3)+1,:));
     else
